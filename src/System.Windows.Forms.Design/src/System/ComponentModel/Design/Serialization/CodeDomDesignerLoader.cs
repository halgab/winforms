...
public abstract partial class CodeDomDesignerLoader : BasicDesignerLoader, INameCreationService, IDesignerSerializationService
{
    ...
    {
        if (GetService<IComponentChangeService>() is { } cs)
        {
            cs.ComponentRemoved -= new ComponentEventHandler(OnComponentRemoved);
            cs.ComponentRename -= new ComponentRenameEventHandler(OnComponentRename);
        }

        if (GetService<IDesignerHost>() is { } host)
        {
    ...
    protected override void Initialize()
    {
        ...
        _extenderProviderService = GetService<IExtenderProviderService>();
    ...
    protected virtual void OnComponentRename(object component, string oldName, string newName)
    {
        ...
        {
            if (members[i] is CodeMemberField codeMemberField
                && codeMemberField.Name.Equals(oldName)
                && codeMemberField.Type.BaseType.Equals(TypeDescriptor.GetClassName(component)))
            {
                codeMemberField.Name = newName;
                break;
            }
        }
    }
    ...
    private static bool TypesEqual(CodeTypeReference typeLeft, CodeTypeReference typeRight)
    {
        ...
        if (typeLeft.TypeArguments is not null)
        {
            return false;
        }
    ...
    {
        if (serializationData is not SerializationStore serializationStore)
        {
        ...
        ComponentSerializationService? css = GetService<ComponentSerializationService>();
        ...
        return css.Deserialize(serializationStore, LoaderHost.Container);
    }
    ...
    string INameCreationService.CreateName(IContainer? container, Type dataType)
    {
        ...
        if (container is not null)
        {
            ...
            while (container.Components[finalName] is not null || memberHash.Contains(finalName));
        }
        else
        {
            finalName = baseName;
